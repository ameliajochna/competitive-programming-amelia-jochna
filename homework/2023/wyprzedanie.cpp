...
struct F {
  __int128_t a, b;
  F(__int128_t a = 0, __int128_t b = 1) : a(a), b(b) {}
  void reduce() {
    __int128_t tmp = std::gcd(a, b);
    a = a / tmp, b = b / tmp;
  }
...
const F operator+(const F &x, const F &y) {
  __int128_t a1 = x.a, b1 = x.b, a2 = y.a, b2 = y.b;
...
std::set<std::pair<F, int> > hitorder;
...
void collision() {
  F t = (*hitorder.begin()).first;
  int ind = (*hitorder.begin()).second;
...
int calcans() {
  ...
  pt = 1;
  while (pt <= n) {  // hitting or overtaking?
    F curot = ot[pt];
    if (hitorder.empty())
      passing();
    else {
