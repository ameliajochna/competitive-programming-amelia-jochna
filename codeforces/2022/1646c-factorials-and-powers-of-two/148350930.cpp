...
void solve() {
  ...
  ll f1 = 1;
  for (ll i = 2; f1 <= n; i++) {
    if (__builtin_popcountll(f1) != 1) fact.pb(f1);
    f1 *= i;
  }
  // dbg(fact);
  ll ans = 1e18;
  for (ll bt = 0; bt < (1 << ss(fact)); bt++) {
    // aktualna bt maska
    ll sum = 0;
    for (int j = 0; j < ss(fact); j++) {
      if ((bt >> j) & 1) {
        sum += fact[j];
      }
    }
    ...
    ll cur = (__builtin_popcountll(left) +
              __builtin_popcountll(bt));  // dbg(left, sum, cur);
    ans = min(ans, cur);
  }
  if (ans == 1e18) {
    cout << "-1\n";
