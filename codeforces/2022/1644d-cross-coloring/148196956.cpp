...
void solve() {
  ...
  cin >> n >> m >> k >> q;
  vector<pii> v(q);
  for (auto& x : v) cin >> x.st >> x.nd;
  set<int> col, row;
  int cnt = 0;
  for (int i = ss(v) - 1; i >= 0; i--) {
    int a = v[i].st, b = v[i].nd;
    if (row.count(a) && col.count(b)) continue;
    if ((row.count(a) && (ss(row) != n)) || (col.count(b) && ss(col) != m) ||
        (!row.count(a) && !col.count(b))) {
      cnt++;
