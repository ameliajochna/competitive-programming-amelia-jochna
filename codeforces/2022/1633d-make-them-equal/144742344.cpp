...
vector<int> dp;
int const INF = (int)1e9, maxD = (int)1e3 + 10;
...
void solve() {
  ...
  vector<int> sum(k + 1, -INF);
  sum[0] = 0;
  for (int i = 0; i < n; i++) {
    for (int j = k; j - x[i] >= 0; j--) {
      if (sum[j - x[i]] == -INF) continue;
      sum[j] = max(sum[j], sum[j - x[i]] + c[i]);
    }
  }
  int maxsum = 0;
  for (int i = 0; i <= k; i++) maxsum = max(sum[i], maxsum);
  cout << maxsum << '\n';
}
...
int main() {
  ...
  for (int i = 1; i < maxD; i++)
    for (int j = 1; j <= i; j++)
      if (i + i / j < maxD) dp[i + i / j] = min(dp[i + i / j], dp[i] + 1);
  int t;
  cin >> t;
  while (t--) solve();
}
