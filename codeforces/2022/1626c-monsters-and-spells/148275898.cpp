...
void solve() {
  ...
  vector<pii> seq(n);
  for (int i = 0; i < n; i++) seq[i] = {k[i] - h[i] + 1, i};
  sort(all(seq));
  vector<pii> cov;  //{pocz, kon}
  for (int i = 0; i < n; i++) {
    int ind = seq[i].nd;
    bool ok = false;
    // dbg(k[ind], h[ind]);
    for (auto x : cov)  // czy juz pokryte
      if (k[ind] >= x.st && k[ind] <= x.nd) ok = true;
    if (ok) continue;
    ok = true;
    for (auto x : cov) {  // czy mozna zrobic nowe pokrycie
      // dbg(x, k[ind]-h[ind]+1);
      if (x.st <= k[ind] - h[ind] + 1 && x.nd >= k[ind] - h[ind] + 1)
        ok = false;
    }
    if (ok) {  // nowe pokrycie
      cov.pb({k[ind] - h[ind] + 1, k[ind]});
      continue;
    }

    // przedluzamy poprzednie pokrycie

    int maxind = -1;
    for (int j = 0; j < ss(cov); j++)
      if (cov[j].nd < k[ind] && (maxind == -1 || cov[j].nd > cov[maxind].nd))
        maxind = j;
